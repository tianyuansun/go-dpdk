...
type sizeT = int //always, we pass into functions with the result of len(slice),  which is int

// just as C's memcpy, make sure the dest and src capcity >= len
// Memcpy can not handle dest and src overlap condition
func Memcpy(dest, src unsafe.Pointer, len sizeT) unsafe.Pointer {
...
func memcpyH(dest, src unsafe.Pointer, len sizeT) unsafe.Pointer {
...
func Memmove(dest, src unsafe.Pointer, len sizeT) unsafe.Pointer {
	if dest == src {
		return dest
	}
...
func Memcmp(dest, src unsafe.Pointer, len sizeT) int {
...
func Memset(dest unsafe.Pointer, ch int8, len sizeT) unsafe.Pointer {
